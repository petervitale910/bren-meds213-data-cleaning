...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snow_cover = ifelse(Snow_cover > 1, NA, Snow_cover))
```
...
species_long <- species_csv %>% 
  select(-Jdate, -Num_observers, -All_obs_reported, -Observer_hours) %>% 
  pivot_longer(Alder_Flycatcher:Yellow_rumped_Warbler, 
               names_to = 'Species', 
               values_to = 'Occurance',
               values_transform = list(Occurance = as.character))
```
